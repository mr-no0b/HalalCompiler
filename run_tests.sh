...
run_valid_stdin() {
    ...
    if ./halalc "$src" "tests/${base}.c" >/dev/null 2>&1; then
        gcc "tests/${base}.c" -o "tests/${base}.out"
        local got
        got="$(printf "%b" "$stdin_data" | "tests/${base}.out" | tr -d '\r')"
        if [[ "$got" == "$expected" ]]; then
            echo "PASS valid(stdin): $src"
            pass_count=$((pass_count + 1))
        else
            echo "FAIL valid(stdin): $src (expected '$expected', got '$got')"
            fail_count=$((fail_count + 1))
        fi
    else
        echo "FAIL valid(stdin): $src (compiler failed)"
        fail_count=$((fail_count + 1))
    fi
}
...
run_valid_stdin "tests/valid_iqra_scanf.halal" "41\n" "41"
run_valid_stdin "tests/valid_iqra_multi.halal" "41 2.5\n" "41
2.5"
...
run_invalid "tests/invalid_iqra_format.halal"
run_invalid "tests/invalid_iqra_count.halal"
